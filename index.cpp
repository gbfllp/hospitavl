...
void searchByPriorityAndName(AVLNode* node, int priority, string name);
...
void inorderIterative(AVLNode* root);
...
void searchByPriorityAndName(AVLNode* node, int priority, string name) {
    ...
    searchByPriorityAndName(node->left, priority, name);
    
    if (node->patient.priority == priority && node->patient.name == name) {
    ...
    searchByPriorityAndName(node->right, priority, name);
}
...
AVLNode* insertNode(AVLNode* node, Patient p) {
    if (node == nullptr) return createNode(p);
...
// Travessia in-order iterativa usando a pilha
void inorderIterative(AVLNode* root) {
    Stack s;
    initStack(s);
...
int main() {
    ...
    do {
        ...
        cout << "4. Buscar paciente por prioridade e nome\n";
        cout << "5. Remover paciente por ID\n";
        cout << "0. Sair\n";
        ...
        if (opcao == 1) {
            ...
            cout << "\nResultados da busca:\n";
            searchByPriorityAndName(root, searchPriority, searchName);
        }
        else if (opcao == 5) {
